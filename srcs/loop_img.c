...
/*   Updated: 2017/11/30 11:05:49 by clegirar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int		fct_mouse(int button, int x, int y, t_struct *strct)
{
	if (button == 5)
		strct->pos_win->pas += 1;
	if (button == 4)
...
static	void 	my_pixel(t_pict *pict, int x, int y, unsigned int color)
{
	unsigned	char	r;
	unsigned	char	g;
	unsigned	char	b;
...
static	int 	do_change(t_struct *strct)
{
	clear_pixels(strct);
	zoom(strct);
	ft_put_pxl(strct);
